...
ON filmes_disney.id = bilheteria.filme_id

ORDER BY bilheteria.rating  LIMIT 1;


SELECT * FROM filmes_disney 
WHERE id = 13;

-- maior faturamanto nacional
...
ON 
	b.filme_id = f.id
ORDER BY b.vendas_internacionais DESC LIMIT 1;
    
SELECT AVG(b.vendas_nacionais) FROM filmes_disney f
INNER JOIN bilheteria b
ON b.filme_id = f.id;

SELECT COUNT(*) FROM filmes_disney f 
INNER JOIN bilheteria b 
ON b.filme_id = f.id
WHERE b.vendas_nacionais > 500000000;
